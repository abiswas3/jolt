...
impl<F: JoltField> EqPolynomial<F> {
    ...
    pub fn mle(x: &[u128], y: &[u128]) -> F {
        assert_eq!(x.len(), y.len());
        x.par_iter()
            .zip(y.par_iter())
            .map(|(x_i, y_i)| {
                F::from_u128(*x_i * y_i)
                    + (F::one() - F::from_u128(*x_i)) * (F::one() - F::from_u128(*y_i))
            })
            .product()
    }
    ...
    pub fn mle_endian<const E1: Endianness, const E2: Endianness>(
        ...
        if E1 == E2 {
            x.r.par_iter()
                .zip(y.r.par_iter())
                .map(|(x_i, y_i)| {
                    // TODO (ARI) : this might be inefficient
                    F::from_u128(*x_i * y_i)
                        + (F::one() - F::from_u128(*x_i)) * (F::one() - F::from_u128(*y_i))
                })
                .product()
    ...
    /// 0..r.len()`.
    pub fn evals_cached(r: &[u128]) -> Vec<Vec<F>> {
    ...
    pub fn evals_cached_rev(r: &[u128]) -> Vec<Vec<F>> {
    ...
    fn evals_serial(r: &[u128], scaling_factor: Option<F>) -> Vec<F> {
    ...
    fn evals_serial_cached(r: &[u128], scaling_factor: Option<F>) -> Vec<Vec<F>> {
    ...
    fn evals_serial_cached_rev(r: &[u128], scaling_factor: Option<F>) -> Vec<Vec<F>> {
    ...
    pub fn evals_parallel(r: &[u128], scaling_factor: Option<F>) -> Vec<F> {
...
mod tests {
    ...
    fn test_evals() {
        ...
        for len in 5..22 {
            let r = (0..len).map(|_| rng.gen()).collect::<Vec<_>>();
