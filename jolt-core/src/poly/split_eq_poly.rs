...
impl<F: JoltField> GruenSplitEqPolynomial<F> {
    ...
    pub fn new(w: &[u128], binding_order: BindingOrder) -> Self {
        match binding_order {
    ...
    pub fn new_for_small_value(
        ...
        let w_E_in_vars: Vec<u128> = w[split_point_x_out..split_point_x_in].to_vec();
    ...
    pub fn bind(&mut self, r: u128) {
        match self.binding_order {
            BindingOrder::LowToHigh => {
                ...
                self.current_scalar *=
                    F::one() - F::from_u128(self.w[self.current_index - 1]) - F::from_u128(r)
                        + F::from_u128(prod_w_r)
                        + F::from_u128(prod_w_r);
            ...
            BindingOrder::HighToLow => {
                ...
                self.current_scalar *=
                    F::one() - F::from_u128(self.w[self.current_index]) - F::from_u128(r)
                        + F::from_u128(prod_w_r)
                        + F::from_u128(prod_w_r);
    ...
    pub fn get_current_w(&self) -> u128 {
...
mod tests {
    ...
    fn bind_low_high() {
        ...
        let w: Vec<u128> = std::iter::repeat_with(|| rng.gen())
            .take(NUM_VARS)
            .collect();
    ...
    fn test_new_for_small_value() {
        ...
        let w1: Vec<u128> = (0..N).map(|i| rng.gen()).collect(); // Use predictable values
        ...
        let w_E_in_vars_expected1: Vec<u128> =
            w1[split_point1_expected1..split_point_x_in_expected1].to_vec(); // w[2..7] = [2,3,4,5,6]
        let mut w_E_out_vars_expected1: Vec<u128> = Vec::new();
        ...
        let expected_E_in1 = EqPolynomial::<Fr>::evals(&w_E_in_vars_expected1);
        ...
        let cached_E_out1 = EqPolynomial::<Fr>::evals_cached(&w_E_out_vars_expected1);
        ...
        let w2: Vec<u128> = (0..N).map(|_| rng.gen()).collect();
        ...
        let w_E_in_vars_expected2: Vec<u128> =
