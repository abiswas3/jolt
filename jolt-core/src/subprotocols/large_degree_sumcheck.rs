...
pub fn compute_eq_mle_product_univariate<F: JoltField>(
    ...
    let eq_coeffs = [
        F::one() - F::from_u128(r_cycle[round]),
        F::from_u128(r_cycle[round] + r_cycle[round]) - F::one(),
    ];
...
impl<F: FieldMulSmall, ProofTranscript: Transcript> LargeDMulSumCheckProof<F, ProofTranscript> {
    ...
    pub fn prove(
        ...
        let mut r: Vec<u128> = Vec::with_capacity(r_cycle.len());
        ...
        for round in 0..r_cycle.len() {
            ...
            *previous_claim = univariate_poly.evaluate_u128(&r_j);
            ...
            eq_factor = eq_factor.mul_1_optimized(F::from_u128(
                (r_cycle[round] + r_cycle[round] - 1) * r_j + (1 - r_cycle[round]),
            ));
...
impl<F: JoltField, ProofTranscript: Transcript> AppendixCSumCheckProof<F, ProofTranscript> {
    ...
    pub fn prove<const D_MINUS_ONE: usize>(
        ...
        let mut w: Vec<u128> = Vec::with_capacity(D * T.log_2());
        drop(_guard);
        drop(span);
        ...
        for j_idx in 0..T.log_2() {
            ...
            for d in 0..D {
                ...
                if d == 0 {
                    ...
                    C_summands[0] = F::from_u128(r_cycle_val);
                    C_summands[1] = F::from_u128(1 - r_cycle_val);
                }
                ...
                *previous_claim = univariate_poly.evaluate_u128(&w_j);
                ...
                C_summands[0] *= F::from_u128(w_j);
                C_summands[1] *= F::one() - F::from_u128(w_j);
...
mod test {
        ...
        for j in 0..T.pow(D as u32) {
            ...
            j_bit_vec
                ...
                .for_each(|(chunk, digit)| {
                    chunk.push((
                        Fr::from_u128(*digit),
                        Fr::from_u32(1) - Fr::from_u128(*digit),
                    ));
                });
    ...
    fn karatsuba_optimization(D: usize, T: usize) {
        ...
        let r_cycle: Vec<u128> = prover_transcript.challenge_vector_u128(T.log_2());
    ...
    fn large_d_optimization_ra_virtualization<const D1: usize>(D: usize, T: usize) {
        ...
        let mut verifier_transcript = KeccakTranscript::new(b"test_transcript");
        verifier_transcript.compare_to(prover_transcript);
        //let _r_cycle: Vec<Fr> = prover_transcript.challenge_vector(T.log_2());
