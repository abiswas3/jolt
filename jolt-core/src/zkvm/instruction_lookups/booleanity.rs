...
impl<F: JoltField> BooleanityProverState<F> {
    fn new(trace: &[RV32IMCycle], G: [Vec<F>; D], r_address: &[u128], r_cycle: &[u128]) -> Self {
...
impl<F: JoltField> SumcheckInstance<F> for BooleanitySumcheck<F> {
    ...
    fn normalize_opening_point(&self, opening_point: &[u128]) -> OpeningPoint<BIG_ENDIAN> {
        ...
        let mut r_big_endian: Vec<u128> = r_address.iter().rev().copied().collect();
